...
app.use(session({
    ...
    //Needs created @ SQL - USE SCRIPT BELOW:
    /*
...
var fullquery= 'SELECT * FROM dbo.AppInsight';
var fullquerytdt= 'Select * FROM dbo.AppInsight WHERE name like \'\%TDT\%\' ORDER BY PCName ASC';
...
var connection = new sql.Connection(config, function(err) {
    ...
    // Query 
        ...
        request.query(fullquerytdt, function(err, recordset) {
            if(!err){  //only runs on no errors
               //console.dir(recordset);
               mongoString.tdt=recordset;
            }
        });
        request.query(fullqueryscm, function(err, recordset) {
            if(!err){  //only runs on no errors
               //console.dir(recordset);
               mongoString.scm=recordset;
            }
        });
        request.query(fullqueryentp, function(err, recordset) {
            if(!err){  //only runs on no errors
               //console.dir(recordset);
               mongoString.entp=recordset;
            }
        });
        request.query(fullquerymsm, function(err, recordset) {
            if(!err){  //only runs on no errors
l               //console.dir(recordset);
               j=0;
               for(i=0;i<recordset.length;i++){
                if(recordset[j].PCName=="INET"){
                    var result = recordset[j];
                    console.dir("Server: " + result.PCName);
                    console.dir("dbName: " + result.dbName);
                } else if(recordset[j].PCName=="INETD"){
                    var result = recordset[j].dbName;
                    console.dir("INETD: " + result);
                } else {
                    console.dir("say whaaaaaaa? " + result.dbName);
                }
                j++;
               }
        ...
        request.query(fullqueryservicePC, function(err, recordset) {
            if(!err){  //only runs on no errors
               ...
               for(i=0;i<recordset.length;i++){
                if(recordset[j].PCName=="INET"){
                    var result = recordset[j].dbName;
                    console.dir("INET: " + result);
                } else if(recordset[j].PCName=="INETD"){
                    var result = recordset[j].dbName;
                    console.dir("INETD: " + result);
                } else {
                    console.dir("say whaaaaaaa?");
                }
                j++;
               }
